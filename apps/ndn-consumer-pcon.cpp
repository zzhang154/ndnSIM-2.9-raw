...
ConsumerPcon::ConsumerPcon()
  ...
  , m_isBicSs(false)
{
  NS_LOG_DEBUG("[PconConsumer] Cwnd: 10 InFlight: 0 HopCount: 1");
}
...
ConsumerPcon::OnData(shared_ptr<const Data> data)
{
  ...
  // Time cur_rtt = m_rtt->AckSeq(SequenceNumber32(sequenceNum));               // 准确的Rtt
...
// ----- add by z2h in 23.10.31
void
ConsumerPcon::ScheduleNextPacket()
{
  if (m_window == static_cast<uint32_t>(0)) {
    ...
    m_sendEvent = Simulator::ScheduleNow(&ConsumerPcon::SendPacket, this);
  }
}

void
ConsumerPcon::ScheduleNextPacket(Time paused_time)
{
  if (!m_sendEvent.IsRunning()) {
    m_sendEvent = Simulator::Schedule(paused_time, &ConsumerPcon::SendPacket, this);             // 默认发送方式
  }
}

void
ConsumerPcon::SendPacket()
{
  if (!m_active)
    return;
  ...
  interest->setMustBeFresh(false);          // z2h 
