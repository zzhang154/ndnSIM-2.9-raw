...
public:
  ...
  public:
    ...
    /** \brief enables Dpccp queue marking support
     *  - 23-04-12 : add by z2h
     *  - dpccp queue mark 
     */ 
    bool allowDpccpQueueMarking = false;

    /** \brief enables Dpccp queue marking support
     *  - 23-04-12 : add by z2h
     *  - dpccp queue mark interval
     */ 
    time::nanoseconds baseDpccpQueueMarkingInterval = 12_ms;
...
private: // send path
  ...
  /** \brief if the send queue is found to be congested, add a dpccp queue mark to the packet
   *         according to CoDel
   *  add by z2h
   */
  void
  checkDpccpQueueMark(lp::Packet& pkt);
