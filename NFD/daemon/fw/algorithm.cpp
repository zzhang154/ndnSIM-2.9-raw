...
bool
isComesFromNewFace(const Face& inFace, const pit::Entry& pitEntry)
{
  auto face_id = inFace.getId();
  return std::any_of(pitEntry.in_begin(), pitEntry.in_end(),
                    [&face_id] (const pit::InRecord& inRecord) {
                      return inRecord.getFace().getId() == face_id;
                    });
}

bool
hasLocalNextHop(const fib::Entry& fibEntry) 
{
  auto nexthops = fibEntry.getNextHops();
  return std::any_of(nexthops.begin(), nexthops.end(),
                    [] (const fib::NextHop& nexthop) {
                      return nexthop.getFace().getScope() == ndn::nfd::FACE_SCOPE_LOCAL;
                    });
}
